...
import { TrendingScreenProps } from '../../types/navigation';

const TrendingScreen: React.FC<TrendingScreenProps> = () => {
  ...
  const loadTrendingQuestions = useCallback(async () => {
    try {
      setLoading(true);
      const questionsData = await ApiService.getTrendingQuestions(20, selectedSubject || undefined);
      setQuestions(questionsData);
    } catch (error) {
      Alert.alert('Error', 'Failed to load trending questions');
    } finally {
      setLoading(false);
    }
  }, [selectedSubject]);
